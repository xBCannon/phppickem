	...
	while ($row = $query->fetch_assoc()) {
		...
		if (is_numeric($row['homeScore']) && is_numeric($row['visitorScore'])) {
            ...
            if ($row['final'] != 0) {
                ...
                } elseif (intval($row['homeScore']) === intval($row['visitorScore']))  {
                    echo '		<td><b><span class="cls1">LIVE: </span></b><img src="images/helmets_small/' . $visitorTeam->teamID . 'R.gif" /> ' . $visitorTeam->teamID . ' ' . $row['visitorScore'] . ' : ' . $row['homeScore'] . ' ' . $homeTeam->teamID . ' <img src="images/helmets_small/' . $homeTeam->teamID . 'L.gif" />' . '</td>' . "\n";
                }
            }
        } else {
            //show time
