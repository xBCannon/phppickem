...
function getCurrentWeek()
{
    ...
    //THIS FUNCTION IS OLD AND OUTDATED
    global $mysqli;
...
function getWeekByScore()
{
    $today = date();
    global $mysqli;
    $sql = "select distinct weekNum from " . DB_PREFIX . "schedule where final <> 1 order by weekNum limit 1";
    $query = $mysqli->query($sql);
    if ($query->num_rows > 0) {
        $row = $query->fetch_assoc();
        $week = $row['weekNum'];
    ...
    $query->free;
    die('Error getting week number by checking for unfinished scores: ' . $mysqli->error);
}
...
function getTeamRecord($teamID) {
    ...
    $sql = "select weekNum, Cast(homeScore AS SIGNED) > Cast(visitorScore AS SIGNED) as gameWon, Cast(homeScore AS SIGNED) = Cast(visitorScore AS SIGNED) as gameTied ";
